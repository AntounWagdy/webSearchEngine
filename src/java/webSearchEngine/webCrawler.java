...
public class webCrawler {
    ...
    public webCrawler(int _max_threads, int _max_pages, int save_rate) throws SQLException {
    ...
    synchronized boolean checkRobotTxt(String urlString) {
        try {
            ...
            if (!RobotHandlers.containsKey(base)) //first create handler if it does not exist
            {
                ...
                Robots_insert.put(base.toString(), H.toString());
            }
    ...
    void update_DB() throws SQLException {
        ...
        /////visited/////////////////////////
        String urlll;
        for (Iterator<URL> it = visited_insert.iterator(); it.hasNext();) {
            urlll = it.next().toString();
        ...
        ///////////////////Robot handler/////////////////
        String host;
        String robot_handler;

        for (Map.Entry<String, String> entry : Robots_insert.entrySet()) {
            host = entry.getKey();
            robot_handler = entry.getValue();
            qm.insertinto_host_handler(host, robot_handler);
        }
         Robots_insert.clear();
        //////////////////downloade page//////////////////
        String url_page;
        ...
        //////////////////to_visit_delete///////////////
         String uuurl;
        for(int i=0; i<to_visit_delete.size()+1; i++)
         {
             uuurl=to_visit_delete.remove().toString();
